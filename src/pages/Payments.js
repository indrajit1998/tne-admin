...
const Payments = () => {
    ...
    const [refundConsignmentId, setRefundConsignmentId] = useState("");
    ...
    const handleManualRefundSubmit = async (e) => {
        e.preventDefault();
        if (!refundConsignmentId || !refundAmount) {
            alert("Please fill in all fields.");
            return;
        }

        const amountInPaise = Math.floor(parseFloat(refundAmount) * 100);
        ...
        try {
            const response = await api.post(
                `/api/v1/admin/initiateRefund/${refundConsignmentId}`,
                { amount: amountInPaise },
                { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
            );
            alert(response.data?.message || "Refund initiated successfully!");
            setIsRefundModalOpen(false);
            setRefundConsignmentId("");
            setRefundAmount("");
        } catch (error) {
            console.error("Error initiating manual refund:", error);
            const errorMessage = error.response?.data?.message || "Refund initiation failed.";
            alert(errorMessage);
        } finally {
        ...
        <div className={styles.mainContainer}>
            ...
            <div className={styles.userDetailsContent}>
                ...
                {isRefundModalOpen && (
                    <div className={styles.modalOverlay}>
                        <div className={styles.modalContent} style={{ maxWidth: '400px', width: '100%' }}>
                            <h3>Manual Payment Refund</h3>
                            <form onSubmit={handleManualRefundSubmit} style={{ textAlign: 'left', marginTop: '15px' }}>
                                <div style={{ marginBottom: '15px' }}>
                                    <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Consignment ID</label>
                                    <input
                                        type="text"
                                        value={refundConsignmentId}
                                        onChange={(e) => setRefundConsignmentId(e.target.value)}
                                        placeholder="Enter consignment database ID"
                                        required
                                        style={{
                                            width: '100%',
                                            padding: '8px',
                                ...
                                <div style={{ marginBottom: '20px' }}>
                                    <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Refund Amount (₹)</label>
                                    <input
                                        type="number"
                                        step="0.01"
                                        value={refundAmount}
                                ...
                                <div className={styles.modalActions} style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                                    ...
                                    <button
                                        ...
                                        disabled={refundSubmitting}
                                    >
                                        {refundSubmitting ? 'Refunding...' : 'Initiate Refund'}
                                    </button>
                                </div>
                            </form>
                        </div>
